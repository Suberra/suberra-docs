...
|```subscription.ended```	|Fired when a user's subscription ends|
|```subscription.past_due```	|Fired when a user's subscription's ```due_date``` is past_due and user hasn't paid successfully|
|```subscription.updated```	|Fired when the subscription is successfully started, after the payment confirmation.|
|```invoice.created```	|Fired when an invoice is created for a new or renewing subscription.|
|```invoice.paid```	|Fired when an invoice is successfully paid - you can provision access to your product when you receive this event and the subscription ```status``` is ```active```.|
|```invoice.payment_failed```	|Fired when a payment fails for an invoice. If payment fails, the invoice ```status``` is set to ```unpaid_retry``` and the subscription ```status``` remains ```active_renewing```. Payment failures also trigger a ```invoice.payment_failed``` event.|
|```invoice.upcoming```	|Fired a few days before the renewal of a subscription. The number of days is based on the number set for Upcoming renewal events in the Merchant Dashboard.|
|```subscription.trial_will_end```	|Fired a few days before a trial ends. When you receive this event, verify that you have a payment method on the customer so you can bill them. Optionally, notify the customer that they will be charged.|	
